...
| app            | poor contrast turns every themed control black on a light background and white on a dark one | One element per poor-contrast site in `src/` — fifteen, across twelve components, each with a Dark and a Light branch — read for a custom property those branches override, plus a sixteenth read for `ActionCta`'s filled variant (Reverse, Close, share), which is a separate `.ctaWithBackground` rule. All foreground (`rgb(34, 34, 34)`) at the default pair, all `rgb(0, 0, 0)` at `#ffffff`/`#eeeeee` (1.16:1), all `rgb(255, 255, 255)` at `#000000`/`#111111` (1.11:1). Each value is painted onto a probe element and read back as `color`, so it compares colours, not spellings: `black` for `#000` passes, `#111` fails, and an unparsable value is returned as written rather than inheriting a colour. Resolved values, not class names, so it survives the variant moving into CSS (CC-004 row 10). Watched failing, 22 September 2026, with `npx vite build` then `npx playwright test -g "poor contrast"` per mutant: each of the 30 Dark and Light branches set to `false` in turn went red, and so did dropping `isPoorContrast` from `ratio.tsx`'s Dark branch, swapping `badge.tsx`'s Dark/Light pair, and deleting the `.ctaWithBackground` block from `.ctaDark`, each diff naming only the site it broke. Not pinned: the `contrast < 3` threshold (only ratios near 1.1 and 12.72 are used); the other properties the same classes set (`--cta-fg-color`, `--badge-fg-color`, `--tooltip-fg-color`, `--input-bg-color`, `--input-outline-color`); and every element after the first of each kind (the other three badges, the foreground inputs and labels). |
...
| ------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ----------------- |
| `test/e2e/extension.spec.ts`          | 21 tests | 8.5–8.9s locally, 4 workers (three runs, 22 September 2026); 18.1s on `ubuntu-latest`, 2 workers, at 20 tests (12 September 2026) | `npm run test:e2e` (after `npm run build`) | 22 September 2026 |
