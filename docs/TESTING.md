...
One suite, one command. `test/e2e/extension.spec.ts` (18 tests, Playwright Test) loads the built
...
- **Clipboard contents.** The share test asserts the live region's text, not what
  `react-copy-to-clipboard` put on the clipboard.
...
| ---------------------------- | -------- | ------------------------------------ | ------------------------------------------ | ---------------- |
| `test/e2e/extension.spec.ts` | 18 tests | 7.3s locally, 4 workers (three runs) | `npm run test:e2e` (after `npm run build`) | 4 September 2026 |

Groups: 1 service worker, 3 content script, 11 app, 3 colour picker (patched manifest).
