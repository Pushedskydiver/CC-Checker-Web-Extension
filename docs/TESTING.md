...
One suite, one command. `test/e2e/extension.spec.ts` (19 tests, Playwright Test) loads the built
...
- **The clipboard's failure path.** What the share button puts on the clipboard _is_ asserted since
  12 September 2026. What is not: the case where the copy command is refused — a host page sending
  `Permissions-Policy: clipboard-write=()`, or a browser that blocks `execCommand`. That is the case
  the guard on `copy()`'s return value in `copy-cta.tsx` exists for, and reproducing it needs
  `document.execCommand` stubbed inside the panel, which no test here does. The hex-copy button's
  value is untested too; only the share button's is.
...
| ---------------------------- | -------- | ------------------------------------ | ------------------------------------------ | ----------------- |
| `test/e2e/extension.spec.ts` | 19 tests | 7.9s locally, 4 workers (three runs) | `npm run test:e2e` (after `npm run build`) | 12 September 2026 |

Groups: 1 service worker, 3 content script, 12 app, 3 colour picker (patched manifest).
